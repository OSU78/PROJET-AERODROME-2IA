...
if ($_SESSION['session_start'] == 1 && $_SESSION['idT'] == 2) {
    ...
    //require_once('INCLUDES/editAvions.php');
    require_once('AVIONS/avionInfo.php');
        ...
        <section class="div-center">
            <nav aria-label="breadcrumb ">
                <ol class="breadcrumb div-margin-left-right">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Nos Avions</li>
                </ol>
            </nav>
            <div>
                <?php
                $listeAvions = new AvionInfo();
                ...
                <div class="col-md-12">
                    ...
                    <div class="table-responsive">
                        ...
                        <div class="form-group">
                            ...
                            <input style="text-align:center;" class=form-control id="search-avion" type="text" value="" placeholder="Rechercher un ou plusieurs utilisateurs">
                        </div>
                        <div id="result-search"></div>
                        <?php

                        $listeAvions->affichageHtml($connection);
                        ?>
                    </div>
                </div>
                ...
                <div class="col-12" style="text-align: center;">
                    <?php
                    if (isset($erreur) && isset($_GET['delete']) && !empty($_GET['delete']) || isset($erreur) && isset($_GET['confirmAvion']) && !empty($_GET['confirmAvion'])) {
                        echo '<br><button type="button" class="btn btn-danger">' . $erreur . "</button>";
                    } else if (isset($msg) && isset($_GET['delete']) && !empty($_GET['delete']) || isset($msg) && isset($_GET['confirmAvion']) && !empty($_GET['confirmAvion'])) {
            ...
            <script>
                ...
                $(document).ready(function() {
                    $('#search-avion').keyup(function() {
                        $('#result-search').html('');
                        var avions = $(this).val();
                        if (avions != "") {
                            $.ajax({
                                type: 'GET',
                                url: 'INCLUDE/recherche_avions.php',
                                data: 'avion=' + encodeURIComponent(utilisateur),
