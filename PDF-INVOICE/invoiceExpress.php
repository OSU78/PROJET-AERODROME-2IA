...
if (isset($sub[0]['nameSub'])) {
    ...
    $invoice->setFrom(['Aérodrome d\'Evreux Normandie', 'Aérodrome d\'Evreux Normandie', '242 Faubourg Saint Antoine', 'Paris 75016', 'France']);
    ...
    $invoice->addItem($service['name'], $service['description'], 1, $service['price'] * 0.21,
        ...
        $service['price']);
    /* Add totals */
    $invoice->addTotal('Prix Total ', $service['price'], true);
    /* Set badge */
    $invoice->addBadge('PAYER');
    /* Add title */
    $invoice->addTitle('REMARQUE');
    /* Add Paragraph */
    $invoice->addParagraph("The services can't be refunded");
    /* Set footer note */
    $invoice->setFooternote('Aérodrome d\'Evreux Normandie');
    /* Render */
    $name = 'bills/';
